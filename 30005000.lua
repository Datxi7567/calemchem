...
task.spawn(function()
    ...
    while true do
        if levelValue.Value >= 5000 and not triggered then
            triggered = true
            STOP = true
...
local function startScript(char)
    ...
    local function getNearestNPC()
        ...
        for _, npc in pairs(workspace:WaitForChild("NPCs"):GetChildren()) do
            -- Chỉ tìm Hallow1-5
            if (npc.Name == "Hallow1" or npc.Name == "Hallow2" or npc.Name == "Hallow3" or npc.Name == "Hallow4" or npc.Name == "Hallow5") and npc:FindFirstChild("HumanoidRootPart") then
    ...
    task.spawn(function()
        while not STOP do
            ...
            if level >= 1 and level <= 5000 and stat > 0 then
