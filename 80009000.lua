...
task.spawn(function()
    ...
    while true do
        if levelValue.Value >= 9000 and not triggered then
            triggered = true
            STOP = true
...
local function startScript(char)
    ...
    local function getNearestNPC()
        ...
        for _, npc in pairs(workspace:WaitForChild("NPCs"):GetChildren()) do
            -- Chỉ tìm Slime1-5
            if (npc.Name == "Slime1" or npc.Name == "Slime2" or npc.Name == "Slime3" or npc.Name == "Slime4" or npc.Name == "Slime5") and npc:FindFirstChild("HumanoidRootPart") then
    ...
    task.spawn(function()
        while not STOP do
            ...
            if level >= 1 and level <= 9000 and stat > 0 then
